...
Hospital Information System Report
Patient and Administration Information System
Author: Computer science students
...
The receptionist accesses the patient registration form via the system’s main interface.
The receptionist enters personal information (e.g., name, date of birth, contact details).
The receptionist provides medical information as required (e.g., medical history, allergies, and current medications).
The receptionist submits the form.
The system validates the information, checking for completeness and consistency.
A new patient record is created and assigned a unique ID, stored in the Patient Database.
The system confirms the successful registration and provides the patient ID for future reference.
...
The patient navigates to the appointment scheduling section within the HIS.
The patient selects a doctor from the available list and chooses a suitable time slot based on availability.
The patient confirms the appointment details, including date, time, and doctor selection.
The system verifies the time slot’s availability to prevent double booking.
The doctor’s schedule is updated to include the new appointment.
The system sends an appointment confirmation to the patient, with details and a reminder.
